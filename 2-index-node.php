<?php
echo "<title>Preparing Environment</title>";

set_time_limit(600); // 10 minutes
// php.ini max_execution_time = 500
// ^$\n

// Array of HLS Manifest URLs
$urls = [
...
$email = $env["X-Auth-Email"];
$key = $env["X-Auth-Key"];
$account_id = $env["account_id"];

// Cloudflare API details
$apiUrlTemplate = "https://api.cloudflare.com/client/v4/accounts/$account_id/stream/%s/downloads";

// echo $email . " " . $key . " " . $account_id;

// Initialize arrays to store successful and failed links
$successfulLinks = [];
$failedLinks = [];

// Function to enable download option for a video
function enableDownload($videoUid, $apiUrlTemplate, $email, $key, $url, &$successfulLinks, &$failedLinks) {
    $apiUrl = sprintf($apiUrlTemplate, $videoUid);
    ...
    if (curl_errno($ch) || $httpCode != 200) {
        ...
        // Log successful link
        $successfulLinks[] = "https://customer-0rjknk9n0a2sedbn.cloudflarestream.com/" . $videoUid . "/downloads/default.mp4?filename=" . $videoUid .  ".mp4";
    }
...
foreach ($urls as $url) {
    preg_match("/\/([^\/]+)\/manifest\/video.m3u8$/", $url, $matches);
    if (isset($matches[1])) {
        $videoUid = $matches[1];
        enableDownload($videoUid, $apiUrlTemplate, $email, $key, $url, $successfulLinks, $failedLinks);
    } else {
        $failedLinks[] = $url; // Directly log the failed extraction attempt
    }
}

// Output the successful and failed links
echo "<h2>Successful Links</h2>";
echo "<p>Copy & Paste these links to Huawei Cloud VOD.</p>";
...
echo "<h2>Failed Links</h2>";
echo "<p>Try again these files.</p>";
echo "<ol>";
