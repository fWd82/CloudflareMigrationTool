...
class ListActiveAsyncInvocationsRequest implements ModelInterface, ArrayAccess
{
    ...
    * @var string
    */
    protected static $openAPIModelName = 'ListActiveAsyncInvocationsRequest';

    /**
    * Array of property to type mappings. Used for (de)serialization
    * functionUrn  函数的URN，详细解释见FunctionGraph函数模型的描述。
    * requests  需要查询的异步请求ID, 多个请求id使用','分割， 最大支持10个请求id查询。如果不指定，默认查询所有异步调用记录
    ...
    */
    public function listInvalidProperties()
    {
        ...
        if ($this->container['functionUrn'] === null) {
            ...
            if (!is_null($this->container['requests']) && (mb_strlen($this->container['requests']) > 640)) {
                $invalidProperties[] = "invalid value for 'requests', the character length must be smaller than or equal to 640.";
            }
            if (!is_null($this->container['requests']) && (mb_strlen($this->container['requests']) < 0)) {
                $invalidProperties[] = "invalid value for 'requests', the character length must be bigger than or equal to 0.";
            }
    ...
    * Sets requests
    *
    * @param string|null $requests 需要查询的异步请求ID, 多个请求id使用','分割， 最大支持10个请求id查询。如果不指定，默认查询所有异步调用记录
    *
    * @return $this
    */
