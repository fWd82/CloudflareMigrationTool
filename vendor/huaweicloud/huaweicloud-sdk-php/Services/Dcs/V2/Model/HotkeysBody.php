...
class HotkeysBody implements ModelInterface, ArrayAccess
{
    ...
    * @var string
    */
    protected static $openAPIModelName = 'HotkeysBody';

    /**
    * Array of property to type mappings. Used for (de)serialization
    * name  key名称
    * type  key类型
    * shard  热key所在的分片，仅在实例类型为集群时支持,格式为ip:port
    * db  热key所在的db
    * size  key的value大小。
    * unit  key大小的单位。type为string时，单位是：byte；type为list/set/zset/hash时，单位是：count
    * freq  表示某个key在一段时间的访问频度，会随着访问的频率而变化。  该值并不是简单的访问频率值，而是一个基于概率的对数计数器结果，最大为255(可表示100万次访问)，超过255后如果继续频繁访问该值并不会继续增大，同时默认如果每过一分钟没有访问，该值会衰减1。
    *
    * @var string[]
    */
    ...
    * Sets shard
    *
    * @param string|null $shard 热key所在的分片，仅在实例类型为集群时支持,格式为ip:port
    *
    * @return $this
    */
    ...
    * Sets freq
    *
    * @param int|null $freq 表示某个key在一段时间的访问频度，会随着访问的频率而变化。  该值并不是简单的访问频率值，而是一个基于概率的对数计数器结果，最大为255(可表示100万次访问)，超过255后如果继续频繁访问该值并不会继续增大，同时默认如果每过一分钟没有访问，该值会衰减1。
    *
    * @return $this
    */
