...
class OriginRequestUrlRewrite implements ModelInterface, ArrayAccess
{
    ...
    * sourceUrl  需要替换的URI。 改写后的URI以正斜线（/）开头的URI，不含http(s)://头及域名。 长度不超过512个字符。 支持通配符\\*匹配，如：/test/\\*_/\\*.mp4。 匹配方式为“所有文件”时，不支持配置参数。
    ...
    * Sets sourceUrl
    *
    * @param string|null $sourceUrl 需要替换的URI。 改写后的URI以正斜线（/）开头的URI，不含http(s)://头及域名。 长度不超过512个字符。 支持通配符\\*匹配，如：/test/\\*_/\\*.mp4。 匹配方式为“所有文件”时，不支持配置参数。
    *
    * @return $this
    */
    ...
    * Sets targetUrl
    *
    * @param string $targetUrl 以正斜线（/）开头的URI，不含http(s)://头及域名。 长度不超过256个字符。 通配符 * 可通过$n捕获（n=1,2,3...，例如：/newtest/$1/$2.jpg）。
    *
    * @return $this
    */
