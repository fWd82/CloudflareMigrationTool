...
class ModifyOttChannelRecordSettingsRecordSettings implements ModelInterface, ArrayAccess
{
    ...
    * @var string
    */
    protected static $openAPIModelName = 'ModifyOttChannelRecordSettings_record_settings';
    ...
    */
    ...
    {
        $invalidProperties = [];
            if (!is_null($this->container['rollingbufferDuration']) && ($this->container['rollingbufferDuration'] > 1209600)) {
                $invalidProperties[] = "invalid value for 'rollingbufferDuration', must be smaller than or equal to 1209600.";
            }
            if (!is_null($this->container['rollingbufferDuration']) && ($this->container['rollingbufferDuration'] < 0)) {
    ...
    *
    * @param int|null $rollingbufferDuration 最大回看录制时长。在此时间段内会连续不断的录制，为必选项  单位：秒。取值为“0”时，表示不支持录制；最大支持录制14天。
    *
    * @return $this
    */
