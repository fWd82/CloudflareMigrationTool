...
class BatchBandwidth implements ModelInterface, ArrayAccess
{
    ...
    * @var string
    */
    protected static $openAPIModelName = 'BatchBandwidth';

    /**
    * Array of property to type mappings. Used for (de)serialization
    * chargeMode  - 功能说明：按流量计费还是按带宽计费。其中IPv6国外默认是bandwidth，国内默认是traffic。取值为traffic，表示流量计费。
    * name  - 功能说明：带宽名称 - 取值范围：1-64个字符，支持数字、字母、中文、_(下划线)、-（中划线）、.（点） - 约束：如果share_type是PER，该参数必须带,如果share_type是WHOLE并且id有值，该参数会忽略。
    * shareType  - 功能说明：带宽类型 - 取值范围：PER，WHOLE。其中IPv6暂不支持WHOLE类型带宽。
    ...
    * Sets name
    *
    * @param string|null $name - 功能说明：带宽名称 - 取值范围：1-64个字符，支持数字、字母、中文、_(下划线)、-（中划线）、.（点） - 约束：如果share_type是PER，该参数必须带,如果share_type是WHOLE并且id有值，该参数会忽略。
    *
    * @return $this
    */
    ...
    * Sets shareType
    *
    * @param string|null $shareType - 功能说明：带宽类型 - 取值范围：PER，WHOLE。其中IPv6暂不支持WHOLE类型带宽。
    *
    * @return $this
    */
    ...
    * Sets size
    *
    * @param int $size - 功能说明：带宽大小 - 取值范围：默认1Mbit/s~2000Mbit/s（具体范围以各区域配置为准，请参见控制台对应页面显示）。 - 约束：share_type是PER，该参数必须带，如果share_type是WHOLE并且id有值，该参数会忽略。 - 注意：调整带宽时的最小单位会根据带宽范围不同存在差异。   - 小于等于300Mbit/s：默认最小单位为1Mbit/s。   - 300Mbit/s~1000Mbit/s：默认最小单位为50Mbit/s。   - 大于1000Mbit/s：默认最小单位为500Mbit/s。
    *
    * @return $this
    */
