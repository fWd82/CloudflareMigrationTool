...
class Storage implements ModelInterface, ArrayAccess
{
    ...
    * name  磁盘类型名称，可能取值如下： - ULTRAHIGH，表示SSD。 - ESSD,表示急速云盘
    * azStatus  其中key是可用区编号，value是规格所在az的状态，包含以下状态： - normal，在售。 - unsupported，暂不支持该规格。 - sellout，售罄。
    * supportComputeGroupType  性能规格，包含以下状态： - normal：通用增强型。 - normal2：通用增强Ⅱ型。 - armFlavors：鲲鹏通用计算增强型。 - armFlavors2Shared：鲲鹏通用计算增强II型（共享型）。
    *
    * @var string[]
    */
    ...
    * Sets name
    *
    * @param string $name 磁盘类型名称，可能取值如下： - ULTRAHIGH，表示SSD。 - ESSD,表示急速云盘
    *
    * @return $this
    */
    ...
    *
    * @param string[]|null $supportComputeGroupType 性能规格，包含以下状态： - normal：通用增强型。 - normal2：通用增强Ⅱ型。 - armFlavors：鲲鹏通用计算增强型。 - armFlavors2Shared：鲲鹏通用计算增强II型（共享型）。
    *
    * @return $this
    */
