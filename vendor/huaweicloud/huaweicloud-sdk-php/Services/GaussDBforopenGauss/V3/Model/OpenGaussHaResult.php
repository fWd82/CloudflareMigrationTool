...
class OpenGaussHaResult implements ModelInterface, ArrayAccess
{
    ...
    * @var string
    */
    protected static $openAPIModelName = 'OpenGaussHaResult';

    /**
    * Array of property to type mappings. Used for (de)serialization
    * mode  GaussDB 分布式模式，返回值为：enterprise（企业版）；主备版，返回值为：centralization_standard(主备版)。
    * replicationMode  备机同步参数。  取值：  GaussDB为“sync”。 说明： - “sync”为同步模式。
    * consistency  GaussDB的预留参数：指定实例一致性类型，取值范围：strong（强一致性） | eventual(最终一致性)。
    * consistencyProtocol  指定副本一致性协议类型，取值范围：quorum | paxos。不填时，默认为quorum。
    *
    * @var string[]
    */
    ...
    * Sets mode
    *
    * @param string $mode GaussDB 分布式模式，返回值为：enterprise（企业版）；主备版，返回值为：centralization_standard(主备版)。
    *
    * @return $this
    */
    ...
    *
    * @param string|null $consistencyProtocol 指定副本一致性协议类型，取值范围：quorum | paxos。不填时，默认为quorum。
    *
    * @return $this
    */
