...
class TransferEnterpriseMultiAccountReq implements ModelInterface, ArrayAccess
{
    ...
    * @var string
    */
    protected static $openAPIModelName = 'TransferEnterpriseMultiAccountReq';

    /**
    * Array of property to type mappings. Used for (de)serialization
    * customerId  企业子账号的客户ID。您可以调用查询企业子账号列表接口，获取响应参数“id”的返回值。
    * amount  现金账户总划拨金额。 单位：元。取值大于0且精确到小数点后2位。
    * transId  交易序列号，用于防止重复提交。 如果接口调用方不传此参数的值，则系统自动生成。如果接口调用方传入此参数的值，请采用UUID保证全局唯一。
    * balanceType  账户类型： BALANCE_TYPE_DEBIT：余额账户（默认）BALANCE_TYPE_CREDIT：信用账户
    * expireTime  账户到期时间，UTC时间，格式为：2016-03-28T14:45:38Z。 只对信用账户有效，用于限制针对有效期到期时间等于该时间的信用账户余额进行拨款，精确到秒。如果查询信用账户可拨款余额的查询结果没有失效时间，表示永久有效，对于这种账本拨款的情况无需填写。
    *
    * @var string[]
    */
    ...
    * Sets amount
    *
    * @param string $amount 现金账户总划拨金额。 单位：元。取值大于0且精确到小数点后2位。
    *
    * @return $this
    */
    ...
    * Sets transId
    *
    * @param string|null $transId 交易序列号，用于防止重复提交。 如果接口调用方不传此参数的值，则系统自动生成。如果接口调用方传入此参数的值，请采用UUID保证全局唯一。
    *
    * @return $this
    */
    ...
    * Sets expireTime
    *
    * @param string|null $expireTime 账户到期时间，UTC时间，格式为：2016-03-28T14:45:38Z。 只对信用账户有效，用于限制针对有效期到期时间等于该时间的信用账户余额进行拨款，精确到秒。如果查询信用账户可拨款余额的查询结果没有失效时间，表示永久有效，对于这种账本拨款的情况无需填写。
    *
    * @return $this
    */
