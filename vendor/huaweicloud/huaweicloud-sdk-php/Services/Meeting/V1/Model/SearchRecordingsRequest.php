...
class SearchRecordingsRequest implements ModelInterface, ArrayAccess
{
    ...
    * @var string
    */
    protected static $openAPIModelName = 'SearchRecordingsRequest';

    /**
    * Array of property to type mappings. Used for (de)serialization
    * userUuid  用户的UUID。 > 仅管理员有权限查询本企业其他用户的会议录制；普通帐号该字段无效，只能查询自己的。
    * offset  指定返回的页面索引。该值必须大于0。 default: 0
    * limit  查询数量。默认是20，最大500条。
    * queryAll  是否查询企业下所有用户的历史会议。 * true：查询所有用户的会议录制 * false：仅查询管理员自己的会议录制 > 仅对企业管理员生效。
    * searchKey  查询条件。会议主题、会议预约人和会议ID等可作为搜索内容。
    * startDate  查询的起始时间戳（单位毫秒）。
    * endDate  查询的截止时间戳（单位毫秒）。
    * sortType  查询结果排序类型。 - ASC_StartTIME：按录制开始时间升序排序 - DSC_StartTIME：按录制开始时间降序排序
    ...
    * Sets userUuid
    *
    * @param string|null $userUuid 用户的UUID。 > 仅管理员有权限查询本企业其他用户的会议录制；普通帐号该字段无效，只能查询自己的。
    *
    * @return $this
    */
    ...
    * Sets offset
    *
    * @param int|null $offset 指定返回的页面索引。该值必须大于0。 default: 0
    *
    * @return $this
    */
    ...
    * Sets queryAll
    *
    * @param bool|null $queryAll 是否查询企业下所有用户的历史会议。 * true：查询所有用户的会议录制 * false：仅查询管理员自己的会议录制 > 仅对企业管理员生效。
    *
    * @return $this
    */
    ...
    * Sets sortType
    *
    * @param string|null $sortType 查询结果排序类型。 - ASC_StartTIME：按录制开始时间升序排序 - DSC_StartTIME：按录制开始时间降序排序
    *
    * @return $this
    */
