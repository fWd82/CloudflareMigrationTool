...
class ServiceCapability implements ModelInterface, ArrayAccess
{
    ...
    * serviceType  **参数说明**：设备的服务类型。 **取值范围**：长度不超过64，只允许中文、字母、数字、以及_?'#().,&%@!-$等字符的组合。
    * properties  **参数说明**：设备服务支持的属性列表。 **取值范围**：数组长度大小不超过500。
    * commands  **参数说明**：设备服务支持的命令列表。 **取值范围**：数组长度大小不超过500。
    * events  **参数说明**：设备服务支持的事件列表。目前暂未支持自定义事件，创建/修改产品时无需定义该字段。 **取值范围**：数组长度大小不超过500。
    ...
    * Sets serviceType
    *
    * @param string $serviceType **参数说明**：设备的服务类型。 **取值范围**：长度不超过64，只允许中文、字母、数字、以及_?'#().,&%@!-$等字符的组合。
    *
    * @return $this
    */
    ...
    * Sets events
    *
    * @param \HuaweiCloud\SDK\IoTDA\V5\Model\ServiceEvent[]|null $events **参数说明**：设备服务支持的事件列表。目前暂未支持自定义事件，创建/修改产品时无需定义该字段。 **取值范围**：数组长度大小不超过500。
    *
    * @return $this
    */
