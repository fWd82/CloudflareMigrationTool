...
class CreateBatchTask implements ModelInterface, ArrayAccess
{
    ...
    * taskType  **参数说明**：批量任务类型。 **取值范围**： - softwareUpgrade: 软件升级任务 - firmwareUpgrade: 固件升级任务 - createDevices: 批量创建设备任务 - deleteDevices: 批量删除设备任务 - freezeDevices: 批量冻结设备任务 - unfreezeDevices: 批量解冻设备任务 - createCommands: 批量创建同步命令任务 - createAsyncCommands: 批量创建异步命令任务 - createMessages: 批量创建消息任务 - updateDeviceShadows：批量配置设备影子任务 - updateDevices：批量更新设备任务
    ...
    */
    public function listInvalidProperties()
    {
        ...
        if ($this->container['taskType'] === null) {
            ...
            if (!preg_match("/softwareUpgrade|firmwareUpgrade|createDevices|deleteDevices|freezeDevices|unfreezeDevices|createCommands|createAsyncCommands|sendEmails|sendSmss|updateDeviceShadows|createMessages|addDevicesToGroup|deviceConfig|updateDevices|deviceJob/", $this->container['taskType'])) {
                $invalidProperties[] = "invalid value for 'taskType', must be conform to the pattern /softwareUpgrade|firmwareUpgrade|createDevices|deleteDevices|freezeDevices|unfreezeDevices|createCommands|createAsyncCommands|sendEmails|sendSmss|updateDeviceShadows|createMessages|addDevicesToGroup|deviceConfig|updateDevices|deviceJob/.";
            }
            if (!is_null($this->container['taskMode']) && !preg_match("/GATEWAY/", $this->container['taskMode'])) {
                $invalidProperties[] = "invalid value for 'taskMode', must be conform to the pattern /GATEWAY/.";
            }
    ...
    * Sets taskMode
    *
    * @param string|null $taskMode **参数说明**：批量任务的模式，当前只支持网关模式，当task_type为firmwareUpgrade，softwareUpgrade支持该参数。软固件升级的场景下，若升级的设备为某个网关的子设备，则平台下发获取版本信息通知和平台下发升级通知将携带task_id（软固件升级批量任务的任务ID）和sub_device_count（批量任务中网关设备包含的升级子设备数量）字段。 **取值范围**：GATEWAY: 网关模式。
    *
    * @return $this
    */
    ...
    * @return object|null
    */
    public function getTaskExtInfo()
    {
        return $this->container['taskExtInfo'];
    }

    /**
    * Sets taskExtInfo
    *
    * @param object|null $taskExtInfo **参数说明**：批量任务额外扩展信息，当task_type为firmwareUpgrade，softwareUpgrade支持该参数。软固件升级的场景下，平台下发获取版本信息通知和平台下发升级通知将携带该字段。 **取值范围**：最长不超过512个字符。
    *
    * @return $this
    */
    public function setTaskExtInfo($taskExtInfo)
    {
        $this->container['taskExtInfo'] = $taskExtInfo;
        return $this;
    }
    ...
    * Sets targetsFilter
    *
    * @param map[string,object]|null $targetsFilter **参数说明**：任务目标筛选参数。Json格式，里面是一个个键值对，（K,V）格式标识筛选targets需要的参数，目前支持的K有group_ids（V填写group_id数组，eg:[\"e495cf17-ff79-4294-8f64-4d367919d665\"]，任务则会筛选出来符合该群组条件的设备作为目标）。当task_type为firmwareUpgrade，softwareUpgrade，deleteDevices，freezeDevices，unfreezeDevices，createCommands，createAsyncCommands，createMessages，updateDeviceShadows，支持该参数。同时使用targets、targets_filter、document_source参数时，只有一个参数会生效，且平台优先使用targets，其次是targets_filter，最后是document_source。
    *
    * @return $this
    */
